...
export function AdvancedSettings({ options, onChange, copies }: AdvancedSettingsProps) {
  ...
  return (
    <div className="card-premium rounded-2xl border border-white/6 overflow-hidden">
      {/* Accordion Toggle */}
      <button
        type="button"
        onClick={() => setIsOpen(!isOpen)}
        className="w-full p-4 flex items-center justify-between text-left hover:bg-white/[0.025] transition-colors group"
      >
        <div className="flex items-center space-x-3">
          <div className="w-7 h-7 rounded-lg bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-center text-indigo-400 group-hover:border-indigo-400/40 transition-colors">
            <Sliders className="w-3.5 h-3.5" />
          </div>
          <div>
            <span className="text-xs font-bold text-white flex items-center gap-1.5">
              Advanced Print Settings
              <span className="text-[10px] font-medium text-slate-500 bg-slate-800/80 px-1.5 py-0.5 rounded border border-white/5">
                Optional
              </span>
            </span>
      ...
      {/* Expandable Panel */}
      {isOpen && (
        <div className="px-4 pb-4 pt-2 border-t border-white/5 space-y-4 animate-fade-in-up text-xs">
          {/* Page Scaling */}
          <div className="space-y-2">
            <label className="text-slate-400 font-semibold flex items-center gap-1.5 text-[11px] uppercase tracking-wider">
                ...
                <button
                  ...
                  className={`p-2.5 rounded-xl text-left border transition-all ${
                    options.scaling === item.id
                      ...
                      : 'border-white/5 bg-slate-900/60 text-slate-400 hover:border-white/12 hover:text-white'
                  }`}
                >
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <select
                ...
                className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-white/10 text-white text-xs focus:outline-none focus:border-indigo-500 transition-colors"
              >
          ...
          {copies > 1 && (
            <div className="pt-2 border-t border-white/5 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <span className="text-white font-semibold text-xs">Collate Copies</span>
              ...
              <button
                ...
                className={`relative w-10 h-6 rounded-full transition-colors duration-200 ${
                  options.collate ? 'bg-indigo-600' : 'bg-slate-700'
                }`}
              >
                <span
