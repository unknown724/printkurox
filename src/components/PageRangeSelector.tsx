...
export function PageRangeSelector({
  ...
  return (
    <div className="card-premium rounded-2xl p-4 space-y-4 border border-white/8">
      {/* Header */}
        ...
        {[
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-xl text-xs font-semibold border transition-all duration-150 ${
              preset.active
                ? 'border-indigo-400/60 bg-indigo-600/25 text-white shadow-sm shadow-indigo-500/20'
                : 'border-white/8 bg-slate-900/50 text-slate-400 hover:border-indigo-500/40 hover:text-slate-200 hover:bg-white/5'
            }`}
          >
            {preset.label}
          </button>
                ...
                <button
                  ...
                  className={`h-7 min-w-[30px] px-2 rounded-lg text-xs font-mono font-bold transition-all duration-100 flex items-center justify-center gap-0.5 select-none ${
                    isSelected
                      ...
                      : 'bg-slate-900/70 text-slate-500 border border-white/5 hover:border-white/20 hover:text-slate-300 line-through decoration-slate-600'
                  }`}
                >
                  <span>{pageNum}</span>
      ...
      <div className="space-y-1.5">
        <div className="flex items-center justify-between text-[11px] text-slate-400 px-0.5">
          ...
          <span className="text-slate-500 font-mono text-[10px]">e.g. 1-3, 5, 8-10</span>
        </div>
        ...
        <div className="relative">
          <input
            ...
            className={`w-full bg-slate-950/80 border rounded-xl px-3 py-2.5 text-xs text-white placeholder-slate-500 font-mono focus:outline-none transition-all pr-8 ${
              !validationResult.isValid
                ? 'border-rose-500/80 focus:border-rose-500 bg-rose-950/10'
                : 'border-white/10 focus:border-indigo-500 focus:bg-slate-950'
            }`}
          />
          {rangeInput && rangeInput.toLowerCase() !== 'all' && (
            <button
              type="button"
              onClick={applyPresetAll}
              className="absolute right-2.5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-white p-0.5 transition-colors"
